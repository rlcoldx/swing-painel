...
(function($){
	...
	return this.each(function(){
		...
		this.opt = $.extend({
			...
			minDate:DateToStr(d),
			date:d,
			daysName:['Dom','Seg','Ter','Qua','Qui','Sex','Sab'],
		...
		let table = $('<table class="table text-center">').appendTo($(this));
		if(this.opt.showCaption){
			this.caption = $('<div class="mes">').insertBefore(table);
			if(this.opt.showArrows){
				this.larr = $('<div><i class="fa-solid fa-arrow-left"></i>').addClass('larr').appendTo(this.caption);
				this.cap = $('<span>').appendTo(this.caption);
				this.rarr = $('<div><i class="fa-solid fa-arrow-right"></i>').addClass('rarr').appendTo(this.caption);
			}
		}else
			this.caption = false;
